    ...
    def _train_epoch(self, epoch):
        ...
        with tqdm(self.data_loader) as progress:
            for batch_idx, (data, label, indexs, _) in enumerate(progress):
                ...
                # plt.imshow(data[0].numpy().squeeze(axis=0))
                # plt.savefig('temp0.jpeg')
                data_max=torch.max(data).detach()
                data_min=torch.min(data).detach()
                data=data+torch.randn(data.shape) * self.config['feature_noise_rate']
                data=torch.clip(data,data_min,data_max)
                # plt.imshow(data[0].numpy().squeeze(axis=0))
                # plt.savefig('temp1.jpeg')
