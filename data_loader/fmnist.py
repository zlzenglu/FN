...
def get_fmnist(root, cfg_trainer, train=True,
                transform_train=None, transform_val=None,
                download=True):
    base_dataset = torchvision.datasets.FashionMNIST(root, train=train, download=download)
    if train:
        ...
        if cfg_trainer['asym']:
            assert 1==0, "pair noise Not suppoted yet"
...
def train_val_split(base_dataset: torchvision.datasets.FashionMNIST):
    num_classes = 10
...
class MNIST_train(torchvision.datasets.FashionMNIST):
...
class MNIST_val(torchvision.datasets.FashionMNIST):
